...
<div id="sidebar" class="fixed left-0 top-[5vh] bg-gray-500 w-[20vw] h-[95vh] flex flex-col opacity-0 z-50 transition-opacity duraction-900">
    ...
    <ul class="absolute bottom-0 flex flex-col text-center text-white w-full flex items-center">
        @if (Auth::check())
            ...
            <x-side-bar-btn method="POST" route="{{ route('logout') }}" name="Sign out"></x-side-bar-btn>
        @else
            <x-side-bar-btn method="GET" route="{{ route('login') }}" name="Log in"></x-side-bar-btn>
            <x-side-bar-btn method="route="/" name="Register"></x-side-bar-btn>
        @endif
    </ul>
</div>
